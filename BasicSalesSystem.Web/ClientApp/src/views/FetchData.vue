...
<script lang="ts">
    ...
    export default class FetchDataView extends Vue {
        ...
        private async fetchWeatherForecasts6() {
            try {
                const response = await axios.get<Forecast[]>('api/WeatherForecast');
                this.forecasts = response.data;
            } catch (e) {
        ...
        private async fetchWeatherForecasts2() {
            try {
                const response = await axios.get<Forecast[]>('api/WeatherForecast');
                this.forecasts = response.data;
            } catch (e) {
        ...
        private async fetchWeatherForecasts3() {
            try {
                const response = await axios.get<Forecast[]>('api/WeatherForecast');
                this.forecasts = response.data;
            } catch (e) {
        ...
        private async fetchWeatherForecasts5() {
            try {
                const response = await axios.get<Forecast[]>('api/WeatherForecast');
                this.forecasts = response.data;
            } catch (e) {
        ...
        async onFiltersChanged(value: any, oldValue: any) {
            var self = this;
            //await self.getCategoriesList();
        }

        @Watch('categoriesTable.options', { immediate: true, deep: true })
        async onOptionsChanged(value: any, oldValue: any) {
            var self = this;
            //await self.getCategoriesList();
        }
    }
